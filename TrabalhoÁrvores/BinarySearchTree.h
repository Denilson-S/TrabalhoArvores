...
void BST_Print_LevelOrder(Bin_Node_Point no, int nivel) {
    if (no == nullptr) {
        return;
    }
    if (nivel == 1) {
        ...
        BST_Print_LevelOrder(no->left, nivel - 1);
        BST_Print_LevelOrder(no->right, nivel - 1);
    }
}
int BST_Get_Height(Bin_Node_Point no) {
    if (no == nullptr) {
        return 0;
    }
    int alturaEsquerda = BST_Get_Height(no->left);
    int alturaDireita = BST_Get_Height(no->right);
    return max(alturaEsquerda, alturaDireita) + 1;
}
void BST_Route_LevelOrder(Bin_Node_Point raiz) {
    int altura = BST_Get_Height(raiz);
    for (int i = 1; i <= altura; i++) {
        cout << "Nivel " << i << ": ";
        BST_Print_LevelOrder(raiz, i);
        cout << endl;
    }
}
