    ...
    void AVL_Route_LevelOrder(AVL_Node_Point root) {

    }
    void imprimirNivel(AVL_Node_Point no, int nivel) {
        if (no == nullptr) {
            return;
        }
        if (nivel == 1) {
    ...
    int calcularAltura(AVL_Node_Point no) {
        if (no == nullptr) {
            return 0;
        }
    ...
    void imprimirPorAltura(AVL_Node_Point raiz) {
