...
//função de percurso por nível
void Bin_Print_LevelOrder(Bin_Node_Point no, int nivel) {
    if (no == nullptr) {
        return;
    }
    if (nivel == 1) {
        ...
        Bin_Print_LevelOrder(no->left, nivel - 1);
        Bin_Print_LevelOrder(no->right, nivel - 1);
    }
}
int Bin_Get_Height(Bin_Node_Point no) {
    if (no == nullptr) {
        return 0;
    }
    int alturaEsquerda = Bin_Get_Height(no->left);
    int alturaDireita = Bin_Get_Height(no->right);
    return max(alturaEsquerda, alturaDireita) + 1;
}
void Bin_Route_LevelOrder(Bin_Node_Point raiz) {
    int altura = Bin_Get_Height(raiz);
    for (int i = 1; i <= altura; i++) {
        cout << "Nivel " << i << ": ";
        Bin_Print_LevelOrder(raiz, i);
        cout << endl;
    }
}
